...
int read_bmp(const char *fname_s) {
  ...
  // move offset to 10 to find rgb raw data offset(BUG here: 138)
...
int gaussian(void) {
  unsigned int x, y, i, v, u, cnt; // for loop counter
...
// cout << int(R) << ", " << int(G) << ", " << int(B) << endl;
              val[0] += double(R) * mask[u + xBound][v + yBound];
              val[1] += double(G) * mask[u + xBound][v + yBound];
              val[2] += double(B) * mask[u + xBound][v + yBound];
            }
          }
        }
        *(image_t + byte_per_pixel * (width * y + x) + 2) = val[0]/cnt;
        *(image_t + byte_per_pixel * (width * y + x) + 1) = val[1]/cnt;
        *(image_t + byte_per_pixel * (width * y + x) + 0) = val[2]/cnt;
    }
  }
...
int main(void) {
  ...
  gaussian();
  assert(write_bmp("lena_gaussian_sw.bmp") == 0);
  delete (image_s);
  delete (image_t);
